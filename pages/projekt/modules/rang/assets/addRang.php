...
if($beschreibung != ""){
    ...
    if($max <= 2){
        echo ("<erro>Die Beschreibung, muss min. 3 Zeichen lang seinen!");
        exit();
    }

    if($max >= 250){
        echo("<erro>Die Beschreibung, darf max. 250 Zeichen lang seinen!");
        exit();
    }
}else {
    echo("<erro>Es muss eine Beschreibung angeben werden.");
    exit();
}
...
if($name != ""){
    ...
    if($max <= 2){
        echo ("<erro>Der Name, muss min. 3 Zeichen lang seinen!");
        exit();
    }

    if($max >= 100){
        echo ("<erro>Der Name, darf max. 100 Zeichen lang seinen!");
        exit();
    }
    ...
    if(!$update){
        ...
        foreach ($sth->fetchAll() as $row) {
            echo ("<erro>Es gibt Bereits einen Rang mit dem Namen!");
            exit();
        }
    }

}else {
    echo ("<erro>Es muss ein Name angeben werden.");
    exit();
}
...
if($color == "#") {
    echo("<erro>Du musst eine Farbe angeben!");
    exit();
}
...
if($update){
    ...
    $sth->bindParam(5, $_SESSION["projekt.aktiv"]);
    $sth->execute();

    $rang = "";

    $sth = $pdo->prepare("SELECT * FROM projekt_rang WHERE Name like ? LIMIT 1");
    $sth->bindParam(1, $name);
    $sth->execute();
    ...
    for ($i = 0; $i < sizeof($pem); $i++) {
        ...
        foreach ($pdo->query("SELECT * FROM projekt_rang_permission WHERE Permission like '$tmp[0]'") as $row) {
            $id = $row["ID"];
